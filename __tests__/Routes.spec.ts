...
test('useParamState hook - URL state synchronization with debouncing', async ({ page }) => {
  ...
  await expect(page.getByTestId('form-input'), 'Input value is the same as typed').toHaveValue(
    'Hello World!',
    {
      timeout: 3000,
    },
...
test('linker', () => {
  ...
  expect(() => parseRelativeLink('https://example.com/some-pathname?param1=1')).toThrowError(
    'The link is URL',
  )
  expect(() => parseRelativeLink('ftp://example.com/some-pathname?param1=1')).toThrowError(
    'The link is URL',
  )
