...
test("URL parameter handling and linker functionality", async ({ page }) => {
	...
	await expect(page.getByTestId("current_url_1")).toHaveText(
		"/?book_param=1&das_ist=das_ist_string",
	);
	await expect(page.getByTestId("current_url_2")).toHaveText("/");
	await expect(page.getByTestId("current_url_3")).toHaveText(
		"/?string_param=params_some",
	);
	...
	await expect(page.getByTestId("current_url_1")).toHaveText(
		"/some-random-page?book_param=1&das_ist=das_ist_string",
	);
	await expect(page.getByTestId("current_url_2")).toHaveText(
		"/some-random-page",
	);
	await expect(page.getByTestId("current_url_3")).toHaveText(
		"/some-random-page?string_param=params_some",
	);
});
...
test("useParamState hook - URL state synchronization with debouncing", async ({
	page,
}) => {
	...
	await expect(page).toHaveURL(
		"/url-state-form?url_change_test_input_value=text_updated_from_nextjs_router",
	);
	...
	await page.goto(
		"/url-state-form?url_change_test_input_value=text_updated_from_browser_router",
	);
	await expect(page).toHaveURL(
		"/url-state-form?url_change_test_input_value=text_updated_from_browser_router",
	);
	...
	await page
		...
		.pressSequentially("not trigger page reset");
	await expect(page.getByTestId("page_value")).toHaveText("2");

	await page.waitForTimeout(1200);

	await page.getByTestId("toggle_page_reset").click();
	await expect(page.getByTestId("page_reset_active")).toHaveText(
		"Page reset active",
	);
...
test("linker", () => {
	...
	expect(parseRelativeLink("/some-pathname?param1=1").asString()).toEqual(
		ss.asString(),
	);
	expect(() =>
		parseRelativeLink("https://example.com/some-pathname?param1=1"),
	).toThrowError("The link is URL");
	expect(() =>
		parseRelativeLink("ftp://example.com/some-pathname?param1=1"),
	).toThrowError("The link is URL");
