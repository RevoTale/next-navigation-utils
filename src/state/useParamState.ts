...
const useParamState = <T>(
  params: ParameterOptions<T>,
  { debounce = defaultDebounceTimer, updateValue, navigate }: ParamsStateOptions = {},
  ...
  const updateQueryValue = useDebouncedCallback((value: T) => {
    ...
    const valueMiddleware =
      updateValue === undefined ? updatedLink : updateValue(updatedLink, linkBuilder)
  ...
  }, [link, params, queryValue, updateQueryValue, value])
