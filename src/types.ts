...
export interface ParamsStateOptions {
	...
	updateValue?: (
		link: RelativeLinkBuilder,
		source: Linker<RelativeURL>,
	) => RelativeLinkBuilder;
	navigate?: {
		scroll?: boolean;
		push?: boolean;
...
export type SetValueCallback<
	R extends URL | RelativeURL,
	N = R extends URL ? AbsoltuteLinkBuilder : RelativeLinkBuilder,
> = <T>(opt: Pick<ParameterOptions<T>, "name" | "encode">, value: T) => N;
export type GetValueCallback = <V>(
	opt: Pick<ParameterOptions<V>, "name" | "decode">,
) => V;
